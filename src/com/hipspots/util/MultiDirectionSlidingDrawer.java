...
public class MultiDirectionSlidingDrawer extends ViewGroup {
	...
	/*
	 * private static final float MAXIMUM_TAP_VELOCITY = 100.0f; private static final float MAXIMUM_MINOR_VELOCITY = 150.0f; private static final
	 * float MAXIMUM_MAJOR_VELOCITY = 200.0f; private static final float MAXIMUM_ACCELERATION = 2000.0f; private static final int VELOCITY_UNITS =
	 * 1000;
	 */
	private static final float MAXIMUM_TAP_VELOCITY = 10.0f;
	private static final float MAXIMUM_MINOR_VELOCITY = 15.0f;
	private static final float MAXIMUM_MAJOR_VELOCITY = 20.0f;
	private static final float MAXIMUM_ACCELERATION = 200.0f;
	private static final int VELOCITY_UNITS = 100;

	private static final int MSG_ANIMATE = 1000;
	private static final int MSG_ANIMATE2 = 1001;
	private static final int ANIMATION_FRAME_DURATION = 1000 / 60;

	private static final int EXPANDED_FULL_OPEN = -10001;
	private static final int COLLAPSED_FULL_CLOSED = -10002;
	private static final int EXPANDED_HALF_OPEN = -10003;
	private static final int EXPANDED_MINI_OPEN = -10004;
	...
	private int mWidthSpecMode, mWidthSpecSize, mHeightSpecMode, mHeightSpecSize;
	private int currentY = 550;
	int maxOffSet;
	int curretTop = -1;
	...
	public MultiDirectionSlidingDrawer(Context context, AttributeSet attrs, int defStyle) {
		...
		HipspotsApplication.setSlidingDrawer(this);
		...
		mBottomOffset = (int) a.getDimension(R.styleable.MultiDirectionSlidingDrawer_bottomOffset, 0);
		mTopOffset = (int) a.getDimension(R.styleable.MultiDirectionSlidingDrawer_topOffset, 400);
		maxOffSet = 5;
		mAllowSingleTap = a.getBoolean(R.styleable.MultiDirectionSlidingDrawer_allowSingleTap, true);
		mAnimateOnClick = a.getBoolean(R.styleable.MultiDirectionSlidingDrawer_animateOnClick, false);
		...
		double drawer_height = 366.67 * scale;
		// currentY = (int) drawer_height;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mWidthSpecMode = widthSpecMode;
		mWidthSpecSize = widthSpecSize;
		mHeightSpecMode = heightSpecMode;
		mHeightSpecSize = heightSpecSize;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		// if (curretTop==-1)
		mTopOffset = currentY;
		curretTop = mTopOffset + handleHeight;
		...
		// content.getMeasuredWidth(), mTopOffset + handleHeight +
		// content.getMeasuredHeight());
		content.layout(0, curretTop, content.getMeasuredWidth(), mTopOffset + handleHeight + content.getMeasuredHeight());

		// content.layout(0, curretTop, content.getMeasuredWidth(), mTopOffset +
		// handleHeight + 300);
		// content.getMeasuredHeight();
		// TODO
		/*
		 * Log.d(LOG_TAG, "content.layout(1)"); Log.d(LOG_TAG, "ContentHeight: " + content.getMeasuredHeight()); Log.d(LOG_TAG, "HandletHeight: " +
		 * handle.getMeasuredHeight()); Log.d(LOG_TAG, "HandlegetTop: " + handle.getTop()); Log.d(LOG_TAG, "Top: " + content.getTop()); Log.d(LOG_TAG,
		...
		mTopOffset = maxOffSet;
		// curretTop = content.getTop();
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if (action == MotionEvent.ACTION_DOWN) {
			HipspotsApplication.getListContainer().setPadding(0, 0, 0, 0);
			mTracking = true;
			HipspotsApplication.getListActivityInstance().resizeListView(1800);
			...
			if (mVertical) {
				...
				// Log.d(LOG_TAG, "--------------top and y:" + y + " , " + top);
				// Log.d(LOG_TAG, "mTouchDelta:" + mTouchDelta);
				prepareTracking(top);
			} else {
	...
	public void limitMovement(double limitTop, double limitBottom, MotionEvent event, float y) {
		if (mTracking) {
			mVelocityTracker.addMovement(event);
			final int action = event.getAction();
			switch (action) {
			case MotionEvent.ACTION_MOVE:
				if (y < (int) limitTop || y > (int) limitBottom)
					break;
			...
			case MotionEvent.ACTION_CANCEL: {
				if (y < (int) limitTop)
					y = (int) limitTop;
				if (y > (int) limitBottom)
					y = (int) limitBottom;
				currentY = (int) y;
				notMove((int) y);
				// Log.d(LOG_TAG, String.valueOf(y));
				HipspotsApplication.getListContainer().setPadding(0, 0, 0, (int) y);
				break;
			}
			}
		}
	...
	// TODO
	public void animateDrawerHeight() {
		...
		notMove(currentY);
		HipspotsApplication.getListActivityInstance().animateScrolltoTop();
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case DisplayMetrics.DENSITY_LOW:
			...
			break;
		case DisplayMetrics.DENSITY_MEDIUM:
			double limitTopM = 36.33 * scale + 0.5;
			double limitBottomM = 390 * scale + 0.5;
			limitMovement(limitTopM, limitBottomM, event, y);
			break;
		case DisplayMetrics.DENSITY_HIGH:
			double limitTopH = 30.33 * scale + 0.5;
			double limitBottomH = 443 * scale + 0.5;
			limitMovement(limitTopH, limitBottomH, event, y);
			break;
		case DisplayMetrics.DENSITY_XHIGH:
			double limitTopX = 33.33 * scale + 0.5;
			double limitBottomX = 503 * scale + 0.5;
			limitMovement(limitTopX, limitBottomX, event, y);
			break;
		}
	...
	public void moveHandle(int position) {
		final View handle = mHandle;
		if (mVertical) {
	...
	private void prepareContent() {
		...
		if (mVertical) {
			...
			// Log.d(LOG_TAG, "handleHeight: " + handleHeight);
			// Log.d(LOG_TAG, "mTopOffset: " + mTopOffset);
	...
	public void animateOpenTo(int i) {// TODO

		final View content = mContent;
		float y = content.getTop();

		final float scale = getResources().getDisplayMetrics().density;
		double limitBottom = 412 * scale + 0.5;
	...
	private void notMove(int position) {
		// ListView lv = (ListView)findViewById(R.id.list_view);
		// LayoutParams params= new FrameLayout.LayoutParams((int) size, (int)
		// size);
		// lv.LayoutParams(10,10);
		final View content = mContent;
		int maxHeight = content.getMeasuredHeight();
		final float scale = getResources().getDisplayMetrics().density;
		double tabHeight = 45 * scale + 0.5;
		double handleHeight = 10 * scale + 0.5;
		double sortTabHeight = 34 * scale + 0.5;
		...
		case DisplayMetrics.DENSITY_LOW:
			...
			break;
		case DisplayMetrics.DENSITY_MEDIUM:
			listHeight = maxHeight - currentY - 20 - (int) sortTabHeight - (int) handleHeight - (int) handleHeight;
			break;
		case DisplayMetrics.DENSITY_HIGH:
			listHeight = maxHeight - currentY - 20 - (int) sortTabHeight - (int) handleHeight - (int) handleHeight;
			break;
		case DisplayMetrics.DENSITY_XHIGH:
			listHeight = maxHeight - currentY - 10 - (int) sortTabHeight - (int) handleHeight - (int) handleHeight;
			break;
		}

		HipspotsApplication.getListActivityInstance().resizeListView(listHeight);
		mHandler.removeMessages(MSG_ANIMATE);
		mHandler.removeMessages(MSG_ANIMATE2);
		mHandler.sendMessageAtTime(mHandler.obtainMessage(MSG_ANIMATE2), mCurrentAnimationTime);
		stopTracking();
	}

	private void dontDoAnimation() {
		mAnimating = false;
		mContent.setVisibility(View.VISIBLE);
		mExpanded = true;
	}
